...
module.exports.initialize = function() {
  this.queryOptionalParamCount = 3;
  ...
  this.onDecorate('Operation', function(context) {
    ...
    operation.optionalParams = _.filter(operation.parameters, function(parameter) { return parameter.required != true; });

    operation.isQuery =
      operation.method == 'GET' &&
      operation.optionalParams.length >= this.queryOptionalParamCount;

    if (operation.isQuery) {
      _.each(operation.optionalParams, function(parameter) { parameter.queryObject = true; });
